...
<?php
    ...
    if(isset($_SESSION['admin']))
    {
        ...
        if(cekUpdateTransaksi($id_akun, "id_login", "proses", "update") != 0){
            cekUpdateTransaksi($id_akun, "id_login", "proses", "seen");
        }
        ...
        if(isset($_GET['id']))
        {
            ...
            mysqli_query($con, "UPDATE produk_transaksi SET stt_transaksi = 'proses' WHERE id_transaksi = '$id_transaksi' ");
            header("Location: penjual_proses.php");
        } 

        // lakukan selesaikan produk dan lempar ke produk selesai
        if(isset($_POST['selesai']))
        {
            ...
            $resPenjual = mysqli_query($con, "SELECT * FROM lastseen_transaksi WHERE id_login = '$id_penjual' AND id_transaksi = '$id' AND stt_transaksi = 'proses'");
            if(mysqli_num_rows($resPenjual) > 0){
                $id_lastseen = mysqli_fetch_assoc($resPenjual)['id_lastseen'];
                mysqli_query($con, "UPDATE lastseen_transaksi SET stt_transaksi = 'selesai' WHERE id_lastseen = '$id_lastseen'");
            }else{
                mysqli_query($con, "INSERT INTO lastseen_transaksi VALUES('','$id_penjual','$id','selesai')");
            }
            // lakukan sesuatu pada notig pembeli
            $resPembeli = mysqli_query($con, "SELECT * FROM lastseen_transaksi WHERE id_login = '$id_pembeli' AND id_transaksi = '$id' AND stt_transaksi = 'proses'");
            if(mysqli_num_rows($resPembeli) > 0){
                $id_lastseen = mysqli_fetch_assoc($resPembeli)['id_lastseen'];
                mysqli_query($con, "UPDATE lastseen_transaksi SET stt_transaksi = 'selesai' WHERE id_lastseen = '$id_lastseen'");
            }else{
                mysqli_query($con, "INSERT INTO lastseen_transaksi VALUES('','$id_pembeli','$id','selesai')");
            }
            // pergi ke proses pesanan dan lakukan update pada tabel produk_transaksi
            header("Location: penjual_selesai.php?id=$id");
        }
        ...
        if(isset($_POST['batal']))
        {
             ...
             if(mysqli_num_rows($resPenjual) > 0){
                 $id_lastseen = mysqli_fetch_assoc($resPenjual)['id_lastseen'];
                 mysqli_query($con, "UPDATE lastseen_transaksi SET stt_transaksi = 'batal' WHERE id_lastseen = '$id_lastseen'");
             }else{
                 mysqli_query($con, "INSERT INTO lastseen_transaksi VALUES('','$id_penjual','$id','batal')");
             }
             // lakukan sesuatu pada notig pembeli
             $resPembeli = mysqli_query($con, "SELECT * FROM lastseen_transaksi WHERE id_login = '$id_pembeli' AND id_transaksi = '$id' AND stt_transaksi = 'proses'");
             if(mysqli_num_rows($resPembeli) > 0){
                 ...
                 mysqli_query($con, "INSERT INTO lastseen_transaksi VALUES('','$id_pembeli','$id','batal')");
             }
             // pergi ke proses pesanan dan lakukan update pada tabel produk_transaksi
            header("Location: penjual_dibatalkan.php?id=$id");
        }
    }
    else
    {
...
<div class="container d-flex flex-column px-0">
    ...
    <div class="nav bg-white d-flex justify-content-between align-items-center px-3 py-3 ">
        <span style="color:seagreen; font-size: 24px;" onclick="window.history.back();"><b class="icon-arrow-left2 mr-2"></b><span class="text-dark">Proses Pesanan</span></span>
    </div>
    <!-- produk pesanan -->
    <div class="p-2" style="margin-bottom: 60px;">
    <?php
        if($stt_akun == "penjual"){
            $result_transaksi = mysqli_query($con, "SELECT * FROM produk_transaksi INNER JOIN produk ON produk_transaksi.id_produk = produk.id_produk INNER JOIN akun_login ON produk_transaksi.id_penjual = akun_login.id_login WHERE id_penjual = '$id_akun' AND stt_transaksi = 'proses' ORDER BY id_transaksi DESC");
        }elseif($stt_akun == "pembeli"){
            $result_transaksi = mysqli_query($con, "SELECT * FROM produk_transaksi INNER JOIN produk ON produk_transaksi.id_produk = produk.id_produk INNER JOIN akun_login ON produk_transaksi.id_penjual = akun_login.id_login WHERE id_pembeli = '$id_akun' AND stt_transaksi = 'proses' ORDER BY id_transaksi DESC");
        }
    ...
    ?>
        ...
        <!-- chat wa tindakan pesanan -->
        <div class="d-flex flex-row">
        <?php if($stt_akun == "penjual"): ?>
            <form action="" method="post" class="w-50">
                <input type="hidden" name="id" value="<?= $transaksi['id_transaksi']; ?>">
                <button type="submit" name="batal" class="w-100 border-0 p-2 d-flex justify-content-center align-items-center bg-white" style="text-decoration: none; color:seagreen;"><span>BATALKAN</span></button>
            </form>
            <form action="" method="post" class="w-50">
                <input type="hidden" name="id" value="<?= $transaksi['id_transaksi']; ?>">
                <button type="submit" name="selesai" class="w-100 border-0 p-2 d-flex  justify-content-center align-items-center text-white" style="text-decoration: none; background:seagreen;">SELESAI</button>
            </form>
        
        <?php elseif($stt_akun == "pembeli"): ?>
            <a href="<?= wa($id_penjual,"Terimakasih pesanan saya sudah diproses. Segera selesaikan pesanan ini",$url_full); ?>" class="w-50 p-2 d-flex justify-content-center align-items-center bg-success text-white" style="text-decoration: none;"><b class="icon-whatsapp mr-2"></b><span>CHAT PENJUAL</span></a>
        <?php endif; ?>
        </div>
    </div>
    <?php  endwhile; ?>
    <?php  else: ?>
    <span class="d-inline-block w-100 text-secondary text-center" style="font-size: small;">Jika ada pesanan baru segera lakukan proses pesanan</span>
    <?php endif; ?>
    </div>
...
<!-- Menu Fixed Bawah -->
<div id="navBottom">
        <div class="nav-bottom fixed-bottom py-2 px-3 d-flex flex-row bg-white justify-content-between align-items-center" style="display: flex; box-shadow: 0px -5px 10px rgba(0,0,0,0.05);">
