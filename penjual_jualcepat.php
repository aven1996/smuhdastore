...
else
{
    ...
    if(isset($_POST['tambah_jualcepat'])){
        $id_produk = $_POST['id_produk'];
        $mulai = $_POST['starttime'];
        $berakhir = $_POST['endtime'];
        $harga_baru = $_POST['harga_baru'];
            ...
            if(mysqli_num_rows($result) > 0){
                mysqli_query($con, "UPDATE produk_jualcepat SET mulai = '$mulai', berakhir = '$berakhir', harga_baru = '$harga_baru' WHERE id_produk = '$id_produk'");
                header("Location: penjual_jualcepat.php");
            }else{
                mysqli_query($con, "INSERT INTO produk_jualcepat VALUES('','$id_produk','$id_akun','$mulai','$berakhir','$harga_baru')");
                header("Location: penjual_jualcepat.php");
            }
    ...
    if(isset($_POST['hapus'])){
        $id = $_POST['id'];
        mysqli_query($con, "DELETE FROM produk_jualcepat WHERE id_jualcepat = '$id'");
...
<div class="container d-flex flex-column px-0">
    ...
    <div class="nav bg-white d-flex justify-content-between align-items-center px-3 py-3 mb-2">
        <a href="admin_penjual.php" style="color:darkcyan; font-size: 24px; text-decoration:none;">
            ...
            <span class="text-dark">Produk Jual Cepat</span>
        </a>
    </div>
    <!-- notiv -->
    <div class="bg-warning p-2 px-3 text-center">
        <span style="font-size: small;">Tawarkan produkmu dengan harga spesial dalam kurun waktu tertentu</span>
    </div>
    <!-- produk -->
    ...
    <div class="bg-white d-flex align-items-center p-3 mb-2 pb-3">
        <div id="btn_pilih_produk" class="d-flex flex-row justify-content-start align-items-center" style="overflow: hidden;">
            ...
            <div class="d-flex flex-column">
                ...
                ?>
                            <span id="tampilan_aja" style="color: darkcyan; font-weight:bold; ">Rp<?= $row['harga_produk_diskon'];?><i style="font-weight: normal; font-size:small; color:darkgrey;"> (harga diskon <?= cekDiskon($id, "diskon"); ?>%)</i></span>
                            <span id="tampilan_aja" style="color: black; font-weight:normal; ">Rp<?= ambil_1_data("produk","id_produk","$id","harga_produk");?><i style="font-weight: normal; font-size:small; color:darkgrey;"> (harga normal)</i</span>
                    <?php else: ?>
                            <span id="tampilan_aja" style="color: darkcyan; font-weight:bold; ">Rp<?= ambil_1_data("produk","id_produk","$id","harga_produk");?><i style="font-weight: normal; font-size:small; color:darkgrey;"> (harga normal)</i</span>
    ...
    <!-- wAKTU MULAI -->
        <div class="bg-white p-2 px-3">
            <span style="color: black; font-style:normal;">Waktu Mulai</span>
        </div>
        <div class="bg-white d-flex align-items-center mb-2 pb-3">
            <input type="date" name="starttime" class="form-control border-0 px-3" value="<?= date("Y-m-d"); ?>" required>
        </div>

    <!-- WAKTU BERAKHIR -->
        <div class="bg-white p-2 px-3">
            <span style="color: black; font-style:normal;">Waktu Berakhir</span>
        </div>
        <div class="bg-white d-flex align-items-center pb-3 mb-2">
            <input type="date" name="endtime" class="form-control border-0 px-3" value="<?= date("Y-m-d"); ?>" required>
        </div>

    <!-- harga baru -->
        <div class="bg-white p-2 px-3">
            <span style="color: black; font-style:normal;">Harga Baru</span>
        </div>
        <div class="bg-white d-flex align-items-center pb-3">
            <input type="text" name="harga_baru" class="form-control border-0 px-3" placeholder="Rp" required>
        </div>

    <!-- button -->
        <button type="submit" name="tambah_jualcepat" class="btn w-100 text-white mb-2" style="background-color: darkcyan;">Simpan</button>
    </form>
 
    <!-- Produk Jual Cepat -->
    <div class="produkrekomendasi container bg-white py-2 px-0">
        <div class="bg-white p-2 px-3">
            <span style="color: black; font-style:normal;"><b>Produk yang dijual cepat</b></span>
        </div>
        ...
        <div class="d-flex flex-row justify-content-center">
            ...
            <?php
                $result_produk = mysqli_query($con, "SELECT * FROM produk_jualcepat WHERE id_penjual = '$id_akun'");
            ...
            <div class="cart-produk-jualcepat d-block pb-2" style="width: 160px;">
                <a href="detil_produk.php?id_produk=<?= $id_produk; ?>&jualcepat" class="d-inline-block text-dark position-relative" style="text-decoration: none;">
                    <!-- blank jika sudah berakhir -->
                    <?php if($produk['berakhir'] < date("Y-m-d")): ?>
                        <div class="position-absolute d-flex flex-column justify-content-center align-items-center" style="top:0; left:0; right:0; bottom:0; background:rgba(0, 0, 0, 0.5); -moz-backdrop-filter: blur(5px); backdrop-filter: blur(5px);">
                            <b class="icon-cross mb-2 text-danger" style="font-size: 42px;"></b>
                            <b class="text-white">Sudah Berakhir</b>
                        </div>
                    <?php endif; ?>
                    <!-- image -->
                    <div class="cov-img-cart d-flex justify-content-center align-items-center" style="width: 160px; height:160px; overflow:hidden;">
                        <img class="h-100" style="width: auto;" src="assets/foto/<?= ambil_1_data("gambar_produk","id_produk","$id_produk","gambar"); ?>" alt="">
                    </div>
                    <!-- nama produk -->
                    <span class="nama_produk d-block" style="color: black; font-style:normal;"><?= ambil_1_data("produk","id_produk","$id_produk","nama_produk"); ?></span>
                    <!-- mulai  -->
                    <span class="px-1" style="display: block;font-size:small;">Mulai: <?= $produk['mulai']; ?></span>
                    <!-- akhir -->
                    <span class="px-1" style="display: block;font-size:small;">Berakhir: <?= $produk['berakhir']; ?></span>
                    <!-- harga -->
                    <b class="harga" style="color: darkcyan; font-style:normal;"><?= rupiah($produk['harga_baru']); ?></b>

                </a>
                <!-- button hapus -->
                <form action="" method="post">
                    <input type="hidden" name="id" value="<?= $produk['id_jualcepat'];?>">
            ...
            <?php endwhile; ?>
            <?php else: ?>
                <span style="font-size: small;">Belum ada produk yang dijual cepat</span>
            <?php endif; ?>
...
<a href="penjual_jualcepat.php" class="d-inline-block mx-3 my-2" style="color:darkcyan; font-size: 24px; text-decoration:none;">
    <b class="icon-arrow-left2 mr-2"></b>
    <b style="color: black; font-weight:normal; font-size: 20px;">Pilih Produk</b>
</a>
<div class="d-flex justify-content-center align-items-center bg-white border-top ">
    ...
    ?>
        <?php while($i = mysqli_fetch_assoc($result)): ?>
            <a style="width:150px; height:300px; color:black; overflow:hidden;  position: relative; margin:10px;" href="penjual_jualcepat.php?id=<?= $i['id_produk']; ?>">
                <!-- diskon --> 
