...
<head>
    ...
    <title>Terakhir Dilihat - Smuhdastore Belaja Nyaman dan Terpercaya</title>
...
<div class="container d-flex flex-column px-0">
    ...
    <div class="nav bg-white d-flex justify-content-between align-items-center px-3 py-3 border-bottom" >
        <a href="admin_pembeli.php" style="color:darkcyan; font-size: 24px; text-decoration: none;" >
            <b class="icon-arrow-left2 mr-2"></b>
            <span class="text-dark">Terakhir Dilihat</span>
        </a>
    </div>
    ...
    <div class="container bg-white p-2 mb-5">
        ...
        <?php
            $res = mysqli_query($con, "SELECT * FROM lastseen INNER JOIN produk ON lastseen.id_produk = produk.id_produk WHERE lastseen.id_login = '$id_akun' ");
            if(mysqli_num_rows($res) > 0):
                while($seen = mysqli_fetch_assoc($res)):
                    $id_produk = $seen['id_produk'];
        ?>
        ...
        <div class="card d-inline-block border-0 position-relative mx-2 mb-2" style="width:160px; height:270px; border-radius: 3px;">
            ...
            <?php endif; ?>
            <!-- nama produk -->
            <span class="d-block mb-2" style="color:black; line-height: 14px; font-size:small; height:30px;"><?= $seen['nama_produk']; ?></span>
            <!-- harga -->
            ...
            <?php endwhile; ?>
        <?php else: ?>
            <span style="font-size: small;">Belum ada produk yang kamu lihat</span>
        <?php endif; ?>
        </div>
        </div>
    </div>
...
<div id="navBottom">
        <div class="nav-bottom fixed-bottom py-2 px-3 d-flex flex-row justify-content-between align-items-center" style="display: flex; box-shadow: 0px -5px 10px rgba(0,0,0,0.05);">
            ...
            <a href="favorit.php" class="text-dark w-25 rounded" style="text-decoration: none; "><div class="d-flex flex-column justify-content-center align-items-center px-2 py-1 position-relative">
